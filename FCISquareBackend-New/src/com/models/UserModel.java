...
public class UserModel {
	...
	public static boolean followUser(Integer userid,Integer followed)
	{
		try{
			Connection conn = DBConnection.getActiveConnection();
			String sql = " Select from following where  'User_ID'= ? , `followed_ID` = ? ";
			PreparedStatement stmt;
			stmt = conn.prepareStatement(sql);
			...
			{
				String Sql = "Insert into following (`User_ID`,`followed_ID') VALUES  (?,?)";
	...
	public static boolean unfollowUser(Integer userid,Integer followed)
	{
		try{
			Connection conn = DBConnection.getActiveConnection();
			String sql = " Select from following where  'User_ID'= ? , `followed_ID` = ? ";
			...
			stmt.setInt(2, followed);
			ResultSet rs = stmt.executeQuery();
			if (!rs.next()) ////lw mesh l2ahom 5las kda kda mesh 3and b3d
				return false ;
			else 
			{
				String Sql = "Delete from following where `User_ID`=?,`followed_ID'=? )";
				PreparedStatement stm;
				stm = conn.prepareStatement(Sql);
				stm.setInt(1, userid);
				stm.setInt(2, followed);
